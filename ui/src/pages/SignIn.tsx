...
const SignIn = () => {
  ...
  const formValidator: FormValidatorType = (
    values: any,
  ): KeyValue<string> | undefined => {
    let result: KeyValue<string> = {};
    if (values.username && values.username.length < usernameLengthMin) {
      result["username"] =
        `Please enter a username of at least ${usernameLengthMin} characters.`;
    }
    if (values.password && values.password.length < passwordLengthMin) {
      result["password"] =
        `Please enter a password of at least ${passwordLengthMin} characters.`;
    }
      ...
      <div className="sign-in-page">
        <div className="sign-in-wrapper">
          <div className="logo-wrapper k-mb-4">
            <img
              width={"100%"}
              src="progress-data-platform.svg"
              alt={"Progress Data Platform"}
            />
          </div>
          <div className="inputs-wrapper k-border k-border-light k-border-solid k-p-4 k-rounded-lg">
            <h2 className="k-mt-0">Email Audit</h2>
            <Form
