...
export const highlightJSON = (
  entity: object | null | Function | string | undefined = {},
  uniqueMatches: string[],
  levelsDown: number = LEVELS_DOWN_DEFAULT,
) => {
...
export const renderDocument = (documentResponse: any, searchResponse: any) => {
  ...
  searchResponse.results
    .find((item) => item.uri === documentResponse.uri)
  ...
  if (uri.endsWith(".json")) {
    return splitAndMap(
      JSON.stringify(highlightJSON(data, uniqueMatches), null, 4),
    );
    ...
    if (part.startsWith(HIGHLIGHT_START)) {
      return (
        <span
          ...
          key={"splitAndMap-" + index}
        >
          {part.substring(15, part.indexOf(HIGHLIGHT_END))}
        </span>
      );
    } else {
